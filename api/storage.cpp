...
DatabaseStorage::DatabaseStorage(const char* encoding,
...
int DatabaseStorage::Connect(const char* host,
                             const char* username,
                             const char* password,
                             const char* database,
  ...
  if (m_bOpened) {
    ...
    if (mysql_real_connect(m_hMySQL, host, username, password, database, port,
                           unix_socket, 0) != NULL) {
...
int DatabaseStorage::Ping() {
  if (m_bOpened) {
    ...
    ret_val = mongoc_client_command_simple(m_hMongoDB, m_database.c_str(),
                                           &ping, NULL, &reply, &error);
    if (ret_val) {
      bson_iter_t iter;
...
void DatabaseStorage::KeepLive() {
  if (Ping() != 0) {
    Close();
    Connect(m_host.c_str(), m_username.c_str(), m_password.c_str(),
            m_database.c_str(), m_unix_socket.c_str(), m_port);
  }
}
