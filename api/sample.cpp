...
void ApiSample::Response() {
  ...
  if (abc == "" || def == "") {
    ...
    strResp =
        "<html><head><title>API Sample</title></head><body>API Sample: "
        "couldn't get the corresponding POST data</body></html>";
    ...
    m_request->set_cookie_var("test1", "value1", 90, NULL, "/", NULL, FALSE,
                              FALSE);
    m_request->set_cookie_var("test2", "value2", 100, NULL, "/", NULL, TRUE,
                              TRUE);
    m_request->set_cookie_var("test3", "value3", 120, NULL, "/",
                              ".uplusware.com", TRUE, TRUE);
    ...
    if (r1 != NULL)
      m_request->set_session_var(
          "session_var1",
          "hello session 1! = ] <script>alert(\"aaaa\")</script>");
    if (r2 != NULL)
    ...
    string strEscapedServerValue1, strEscapedServerValue2,
        strEscapedServerValue3;
