...
bool get_file_etag(const char* path,
                   unsigned char etag[33],
                   char* filebuf = NULL) {
  char* pbuf = filebuf;
...
bool get_file_length_lastmidifytime(const char* path,
                                    long long& length,
...
void web_doc::Response() {
  ...
  if (!m_session->RequestNoCache() &&
      m_session->m_cache->_find_file_(strResourceFullPath.c_str(),
                                      &file_cache_instance) &&
      file_cache_instance) {
  ...
  if (file_cache_instance && file_cache_data) {
    ...
    if (cache_time_out < 10 && cache_time_out > 0) {
      ...
      // 90s timeout
      if (get_file_length_lastmidifytime(strResourceFullPath.c_str(),
                                         nResourceLength,
                                         tLastModifyTime) == true &&
          tLastModifyTime == file_cache_data->t_modify && cache_time_out < 90 &&
          cache_time_out > 0) {
        file_cache_available = true;
      } else {
  ...
  if (strcmp(m_session->GetRequestField("Cache-Control"), "no-cache") != 0 &&
      ...
      strcmp(m_session->GetRequestField("If-Modified-Since"),
             strDateTime.c_str()) == 0) {
    /* No such file */
    CHttpResponseHdr header(m_session->GetResponseHeader()->GetMap());
    ...
    if (file_cache_available) {
      ...
      if (get_file_etag(strResourceFullPath.c_str(), szETag, file_buf) ==
          false) {
        if (file_buf)
          delete[] file_buf;
      ...
      if (file_buf) {
        ...
        bool push_rc = m_session->m_cache->_push_file_(
            strResourceFullPath.c_str(), file_buf, nResourceLength,
            tLastModifyTime, szETag, &file_cache_instance);
        m_session->m_cache->unlock_file_cache();
  ...
  if (file_cache_buf) {
    ...
    if (strcasecmp(strExtName.c_str(), "css") == 0 ||
        strcasecmp(strExtName.c_str(), "js") == 0) {
    ...
    if (m_session->m_cache->m_type_table.find(strExtName) ==
        m_session->m_cache->m_type_table.end()) {
      header.SetField("Content-Type", "application/*");
    } else {
      header.SetField("Content-Type",
                      m_session->m_cache->m_type_table[strExtName].c_str());
    }
    if (strcmp(szHttpRequestRanges, "") != 0 && strContentRangeResponse != "") {
        ...
        if (m_session->GetMethod() != hmHead) {
          ...
          if (m_session->GetMethod() != hmHead)
            m_session->SendContent(file_cache_buf + content_ranges[v].beg,
                                   rest_length > content_ranges[v].len
                                       ? content_ranges[v].len
                                       : rest_length);
        }
      }
    }
