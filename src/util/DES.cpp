 ...
 * Brian Sheng ports to a class. uplusware@gmail.com
 */
#include <stdlib.h> 
#include <stdio.h>
#include "DES.h"
...
/* Byte swap a int */
unsigned int DES::byteswap(unsigned int x)
{
...
void DES::perminit(char perm[16][16][8], char p[64])
{
	register int l, j, k;
	int i,m;
...
int DES::Init(int mode)
{
	...
	if((sp_ = (int (*)[64])malloc(sizeof(int) * 8 * 64)) == NULL){
		return -1;
	}
	spinit();
...
void DES::permute(char * inblock,char perm[16][16][8],char *outblock)
{
	register int i,j;
...
int DES::f(unsigned int r,unsigned char subkey[8])
{
...
void DES::round(int num,unsigned int *block)
{
