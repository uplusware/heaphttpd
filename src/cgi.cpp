...
int cgi_base::Connect() {
  ...
  m_sockfd =
      socket(m_sockType == INET_SOCK ? AF_INET6 : AF_UNIX, SOCK_STREAM, 0);
  if (m_sockfd < 0) {
  ...
  if (m_sockType == INET_SOCK) {
    ...
    int s = getaddrinfo(m_strIP != "" ? m_strIP.c_str() : NULL, szPort, &hints,
                        &server_addr);
    if (s != 0) {
    ...
    sprintf(local_sockfile, "/tmp/heaphttpd/fastcgi.sock.%05d.%05d", getpid(),
            gettid());
    ...
    int size =
        offsetof(struct sockaddr_un, sun_path) + strlen(ser_unix.sun_path);
  ...
  getsockopt(m_sockfd, SOL_SOCKET, SO_ERROR, (char*)&err_code,
             (socklen_t*)&err_code_len);
  if (err_code != 0) {
    close(m_sockfd);
    m_sockfd = -1;
    err_msg = "system error\r\n";
...
int cgi_base::Send(const char* buf, unsigned long long len) {
  return m_sockfd >= 0 ? _Send_(m_sockfd, (char*)buf, len,
                                CHttpBase::m_connection_idle_timeout)
                       : -1;
}

int cgi_base::Recv(const char* buf, unsigned long long len) {
  return m_sockfd >= 0 ? _Recv_(m_sockfd, (char*)buf, len,
                                CHttpBase::m_connection_idle_timeout)
                       : -1;
}
...
forkcgi::forkcgi(int sockfd) : cgi_base(sockfd) {}
