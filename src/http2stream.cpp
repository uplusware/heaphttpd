...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// http2_stream
http2_stream::http2_stream(int epoll_fd,
                           map<int, backend_session*>* backend_list,
                           uint_32 stream_ind,
                           uint_32 local_window_size,
                           uint_32 peer_window_size,
                           CHttp2* phttp2,
  ...
  m_http1 = new CHttp(
      m_epoll_fd, m_backend_list, m_connection_first_request_time,
      m_connection_keep_alive_timeout, m_connection_keep_alive_request_tickets,
      m_http_tunneling, m_php_fpm_instance, m_fastcgi_instances,
      m_scgi_instances, m_srvobj, m_sockfd, m_servername.c_str(), m_serverport,
      m_clientip.c_str(), m_client_cert, m_ch, m_work_path.c_str(),
      m_default_webpages, m_ext_list, m_reverse_ext_list, m_php_mode.c_str(),
      m_fpm_socktype, m_fpm_sockfile.c_str(), m_fpm_addr.c_str(), m_fpm_port,
      m_phpcgi_path.c_str(), m_cgi_list, m_private_path.c_str(),
      m_wwwauth_scheme, m_proxyauth_scheme, m_ssl, m_http2, m_stream_ind);
  m_hpack = NULL;
...
void http2_stream::BuildPushPromiseResponse(http2_stream* host_stream,
                                            const char* path) {
  ...
  if (strcmp(host_stream->GetHttp1()->GetRequestField("Accept"), "") != 0) {
    ...
    m_push_promise_trigger_header +=
        host_stream->GetHttp1()->GetRequestField("Accept");
    m_push_promise_trigger_header += "\r\n";
  }
  if (strcmp(host_stream->GetHttp1()->GetRequestField("Accept-Encoding"), "") !=
      0) {
    m_push_promise_trigger_header += "Accept-Encoding: ";
    m_push_promise_trigger_header +=
        host_stream->GetHttp1()->GetRequestField("Accept-Encoding");
    m_push_promise_trigger_header += "\r\n";
  }
  if (strcmp(host_stream->GetHttp1()->GetRequestField("Accept-Language"), "") !=
      0) {
    m_push_promise_trigger_header += "Accept-Language: ";
    m_push_promise_trigger_header +=
        host_stream->GetHttp1()->GetRequestField("Accept-Language");
    m_push_promise_trigger_header += "\r\n";
  }
  if (strcmp(host_stream->GetHttp1()->GetRequestField("User-Agent"), "") != 0) {
    m_push_promise_trigger_header += "User-Agent: ";
    m_push_promise_trigger_header +=
        host_stream->GetHttp1()->GetRequestField("User-Agent");
    m_push_promise_trigger_header += "\r\n";
  }
  m_push_promise_trigger_header += "\r\n";
}
...
void http2_stream::SetDependencyStream(uint_32 dependency_stream) {
  if (dependency_stream == 0)
    ...
    http2_stream* p_dependency_stream =
        m_http2->get_stream_instance(dependency_stream);
...
void http2_stream::IncreasePeerWindowSize(uint_32 window_size) {
  ...
  //    {
  //        http2_stream* p_dependency_stream =
  //        m_http2->get_stream_instance(m_dependency_stream);
  //        if(p_dependency_stream)
...
void http2_stream::DecreaseLocalWindowSize(uint_32 window_size) {
  ...
  if (m_local_window_size <= 0) {
    m_http2->send_window_update(m_stream_ind,
                                m_http2->get_initial_local_window_size());
  }
  //    }
  //    else
  //    {
