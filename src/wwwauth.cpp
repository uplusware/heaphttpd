...
bool WWW_Auth(CHttp* psession,
  ...
  if (scheme == asBasic) {
    ...
    CBase64::Decode((char*)strauth.c_str(), strauth.length(), user_pwd_out,
                    &outlen);
    ...
    if (integrate_local_users) {
      if (strcasecmp(username.c_str(), "root") ==
          0)  // forbid root for login for security reason.
      {
        return false;
      }
      // Get shadow password.
      ...
      char* pwd_hashed =
          crypt_r(password.c_str(), spw_info->sp_pwdp, &pwd_data);
      ...
      if (heaphttpd_usrdef_get_password(psession, username.c_str(),
                                        real_password) &&
    ...
    if (!heaphttpd_usrdef_get_password(psession, DigestMap["username"].c_str(),
                                       real_password))
      return false;
