    ...
    if (pRootElement) {
      ...
      while (pChildNode) {
        if (pChildNode && pChildNode->ToElement()) {
          ...
          bal_svr.host = pChildNode->ToElement()->Attribute("host")
                             ? pChildNode->ToElement()->Attribute("host")
                             : "";
          if (bal_svr.host != "") {
            bal_svr.weight =
                (pChildNode->ToElement()->Attribute("weight") &&
                 pChildNode->ToElement()->Attribute("weight")[0] != '\0')
                    ? atoi(pChildNode->ToElement()->Attribute("weight"))
                    : 8;
...
void* ext_request(CHttp* http_session,
  ...
  strObjName += name;
  http_server_group* svr_grp =
      (http_server_group*)http_session->GetServiceObject(strObjName.c_str());
  ...
  if (svr_grp->m_http_servers.size() > 0) {
    int index = http_session->m_clientip == ""
                    ? 0
                    : http_session->m_clientip
                          .c_str()[http_session->m_clientip.length() - 1];
    ...
    http_session->SendContent(header.GetDefaultHTML(),
